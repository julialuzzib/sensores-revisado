...
def job_leitura_automatica():
    try:
        ...
        for s in data["sensores"]:
            ...
            elif s["modo"] == "estufa" and s["tipo"] == "umid":
                ultimo = s.get("ultimo_valor")
                base = ultimo if ultimo is not None else random.uniform(s["min_val"], s["max_val"])
                
                # Aplica o efeito acumulado da irrigação nas futuras leituras automáticas
                efeito = s.get("efeito_irrigacao", 0)
                if efeito > 0:
                    base = min(base + efeito, 100)
                    s["efeito_irrigacao"] = 0 # Consome o efeito após aplicar para as leituras subsequentes
                
                # Evaporação natural após verificar a irrigação
                valor_base = max(0, base - random.uniform(0.5, 2.0))
                
            elif s["modo"] == "estufa" and s["tipo"] == "temp":
                ultimo = s.get("ultimo_valor")
                base = ultimo if ultimo is not None else config.get("estufa", {}).get("temp_media", 24)
                valor_base = base + random.uniform(-0.5, 0.5)
        ...
        salvar_dados(data)
        print(f"[{agora}] Leitura automática processada.")
...
def irrigacao_sensor(id_sensor):
    ...
    entrada = {"ts": datetime.now().isoformat(), "sensor_id": id_sensor, "tipo": "individual"}
    data.setdefault("historico_irrigacao", []).append(entrada)
    
    # Impacta as leituras futuras guardando o incremento de umidade
    if sensor["tipo"] == "umid" and sensor["modo"] == "estufa":
        sensor["efeito_irrigacao"] = min(sensor.get("efeito_irrigacao", 0) + 30, 100)
...
def registrar_irrigacao():
    ...
    # Primeiro mapeia os alertas com base no último estado estável conhecido
    for s in data["sensores"]:
    ...
    # Aplica o ganho de umidade para impactar estritamente as leituras futuras
    for s in data["sensores"]:
        if s["tipo"] == "umid" and s["modo"] == "estufa":
            s["efeito_irrigacao"] = min(s.get("efeito_irrigacao", 0) + 30, 100)
